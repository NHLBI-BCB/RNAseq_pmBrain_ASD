...
## To submit this script: qsub -cwd -o code_output_M2 -e code_error_M2 -S /bin/bash -V -N Submit_M2 -q geschwind.q -l h_data=8G,h_rt=1:00:00 5_jillSub.sh
...
CODEDIR=/geschwindlabshares/RNAseq_ASD_pancortical/ASDPanBatch1/code/VCF/getVCF
SOURCEDIR=/geschwindlabshares/RNAseq_ASD_pancortical/ASDPanBatch1/source/fails
WORKDIR=/geschwindlabshares/RNAseq_ASD_pancortical/ASDPanBatch1/data/vcfQC_processed
GROUP=group.txt
JOBNAME=jrh_vcf			## a unique identifier for this set of jobs
...
while read groups;do
	...
	if [ "$groups" = "batch01.txt" ]; then
		...
		qsub -cwd -o $WORKDIR/log2 -e $WORKDIR/log2 -V -S /bin/bash -q geschwind.q -N QC2_${names}_${JOBNAME}_${num} -l h_data=36G,h_rt=24:00:00 ${CODEDIR}/6_2pass_2_jill.sh $processdir $names $WORKDIR
		...
		hold_id=$((10#$num -1))
                ...
                qsub -hold_jid *${JOBNAME}*${hold_id}  -cwd -o $WORKDIR/log2 -e $WORKDIR/log2 -V -S /bin/bash -q geschwind.q -N QC2_${names}_${JOBNAME}_${num} -l h_data=36G,h_rt=24:00:00 ${CODEDIR}/6_2pass_2_jill.sh $processdir $names $WORKDIR
