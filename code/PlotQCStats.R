...
setwd("/hp_shares/mgandal/projects/BrainGVEX/processed/QC")
...
colnames(picard.rnaseq.qc) <- c("FileName","PF_BASES","PF_ALIGNED_BASES","RIBOSOMAL_BASES","CODING_BASES","UTR_BASES","INTRONIC_BASES","INTERGENIC_BASES","IGNORED_READS","CORRECT_STRAND_READS","INCORRECT_STRAND_READS",
                                "PCT_RIBOSOMAL_BASES","PCT_CODING_BASES","PCT_UTR_BASES","PCT_INTRONIC_BASES","PCT_INTERGENIC_BASES","PCT_MRNA_BASES","PCT_USABLE_BASES",
...
quantile(picard.rnaseq.qc[,"PF_BASES"]/100) 
quantile(picard.rnaseq.qc[,"CODING_BASES"]/100) 
quantile(picard.rnaseq.qc[,"INTRONIC_BASES"]/100) 
quantile(picard.rnaseq.qc[,"INTERGENIC_BASES"]/100)   
quantile(picard.rnaseq.qc[,"RIBOSOMAL_BASES"]/100)
...
picard.rnadups.qc <- read.table(file="./RNAseqDuplication.txt",sep=" ")
picard.rnadups.qc = picard.rnadups.qc[,-2]
colnames(picard.rnadups.qc) = c("Library","UNPAIRED_READS_EXAMINED","READ_PAIRS_EXAMINED","UNMAPPED_READS","UNPAIRED_READ_DUPLICATES",
                                "READ_PAIR_DUPLICATES","READ_PAIR_OPTICAL_DUPLICATES","PERCENT_DUPLICATION","ESTIMATED_LIBRARY_SIZE")
summary(picard.rnadups.qc)

## More GC metrics
gc.summary <- read.table("./GCsummary.txt",row=1,header=TRUE)
#colnames(gc.summary)=c("WINDOW_SIZE","TOTAL_CLUSTERS","ALIGNED_READS","AT_DROPOUT","GC_DROPOUT")
#colnames(gc.summary)=as.character(gc.summary[1,])
#gc.summary=gc.summary[-1,]
summary(gc.summary)

## Compile most important QC metrics
PQCdat <- cbind(picard.align[,c(2,6,9,21)],picard.rnaseq.qc[,c(3:8,10,21:23)],
                gc.summary[,c(4,5)],picard.rnadups.qc[,c(7,8)])
