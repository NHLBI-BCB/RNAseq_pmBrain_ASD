...
GROUP=group.txt
JOBNAME=jrh			## a unique identifier for this set of jobs
...
while read groups;do
	...
	if [ "$groups" = "batch01.txt" ]; then
		...
		qsub -cwd -o $WORKDIR/log2 -e $WORKDIR/log2 -V -S /bin/bash -q geschwind.q -N QC2_${names}_${JOBNAME}_${num} -l h_data=36G,h_rt=24:00:00 ${CODEDIR}/4_QC.sh $processdir $names $WORKDIR
		...
		hold_id=$((10#$num -1))
                ...
                qsub -hold_jid *${JOBNAME}*${hold_id}  -cwd -o $WORKDIR/log2 -e $WORKDIR/log2 -V -S /bin/bash -q geschwind.q -N QC2_${names}_${JOBNAME}_${num} -l h_data=36G,h_rt=24:00:00 ${CODEDIR}/4_QC.sh $processdir $names $WORKDIR
